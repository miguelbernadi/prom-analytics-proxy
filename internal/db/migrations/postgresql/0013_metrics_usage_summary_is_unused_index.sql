...
-- predicate matches the is_unused filter used by the unused-metrics query
-- path, so the planner can satisfy the scan from this index instead of
-- walking the full summary table.
--
...
-- deadlocked in production: a concurrent INSERT from the inventory syncer held a
